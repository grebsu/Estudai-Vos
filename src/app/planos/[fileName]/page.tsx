...
export default function PlanoDetalhes() {
  ...
  const calculateSubjectStats = useCallback((plan: PlanData | null, records: StudyRecord[]) => {
    ...
    plan.subjects.forEach(subject => {
      ...
      // Tópicos estudados: verificar se há algum registro de estudo para o tópico
      const studiedTopicTexts = new Set(records
        .filter(record => record.subject === subject.subject)
        .map(record => record.topic)
      );
      studiedTopics = Array.from(studiedTopicTexts).filter(topicText => 
        subject.topics.some(t => t.topic_text === topicText)
      ).length;
  ...
  const handleSaveSubject = async (subjectName: string, topics: string[], color: string) => {
    if (!planData) return;

    let updatedSubjects: Subject[];
    let successMessage: string;

    const newTopicsFormatted = topics.map((topicText, index) => ({
      topic_number: `TEMP.${index + 1}`, // Temporário, será reindexado se for nova disciplina
    ...
    if (subjectToEdit) {
      ...
      updatedSubjects = planData.subjects.map(s =>
        s.subject === subjectToEdit.subject
          ? { ...s, subject: subjectName, topics: newTopicsFormatted, color: color }
          : s
      );
    ...
    // Reindexar tópicos após a atualização/adição para garantir topic_number correto
    const reindexedSubjects = updatedSubjects.map((subj, subjIndex) => ({
      ...subj,
      topics: subj.topics.map((topic, topicIndex) => ({
        ...topic,
        topic_number: `${subjIndex + 1}.${topicIndex + 1}`,
      })),
    }));
  ...
  const handleAddTopics = (newTopics: string[], shouldContinue: boolean) => {
    if (!planData || selectedSubjectIndex === null) return;

    const newTopicsFormatted = newTopics.map((topicText, index) => ({
      topic_number: `${selectedSubjectIndex + 1}.${planData.subjects[selectedSubjectIndex].topics.length + index + 1}`,
      topic_text: topicText,
    }));

    const updatedSubjects = [...planData.subjects];
    updatedSubjects[selectedSubjectIndex].topics.push(...newTopicsFormatted);
    ...
    <>
            ...
            {planData.subjects && planData.subjects.map((subject, subjectIndex) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <button
                      onClick={() => router.push(`/materias/${encodeURIComponent(subject.subject)}?plan=${fileName}`)}
      ...
      {planData.subjects && (
        <AddTopicModal
          ...
          existingTopics={selectedSubjectIndex !== null ? planData.subjects[selectedSubjectIndex].topics.map(t => t.topic_text) : []}
          key={`topic-modal-${selectedSubjectIndex}`}
        />
      )}
