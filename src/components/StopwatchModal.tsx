...
const StopwatchModal: React.FC<StopwatchModalProps> = ({ isOpen, onClose, onSaveAndClose, targetDuration = 0, subject: initialSubject }) => {
  ...
  const topics = useMemo(() => {
    ...
    return subjectData ? subjectData.topics.map((t: any) => t.topic_text) : [];
  ...
  useEffect(() => {
    if (mode === 'timer') {
      ...
      setTime(initialMs);
      setInitialTimerTime(initialMs); // Set initial time for progress calculation
      setDisplayTimeInput(formatTime(initialMs));
    } else {
      setTime(0);
      setInitialTimerTime(0); // Reset for cronometro
      setDisplayTimeInput(formatTime(0));
    }
  }, [mode, targetDuration]);
  ...
  const handleSaveAndClose = () => {
    setIsRunning(false);
    const elapsedTime = mode === 'timer' ? (initialTimerTime - time) : time;
