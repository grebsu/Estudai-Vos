...
interface AddTopicModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (topics: string[], shouldContinue: boolean) => void;
...
const AddTopicModal: React.FC<AddTopicModalProps> = ({ isOpen, onClose, onAdd, existingTopics, isViewMode }) => {
  ...
  const handleAdd = () => {
    const newTopics = content.split('\n').map(t => t.trim()).filter(t => t);
    if (newTopics.length > 0) {
      onAdd(newTopics, shouldContinue);
              ...
              {existingTopics.length > 0 ? (
                <ul className="list-disc list-inside">
                  {existingTopics.map((topic, index) => (
                    <li key={index} className="text-gray-700 dark:text-gray-100">{topic}</li>
                  ))}
                </ul>
              ) : (
