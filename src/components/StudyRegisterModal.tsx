...
import AddReviewModal from './AddReviewModal'; // Importar o novo modal
...
interface StudyRegisterModalProps {
  ...
  initialTime?: number;
  justification?: string | null; // Nova prop
  showDeleteButton?: boolean;
}

// Helper function to parse and display the justification string
const JustificationDetails: React.FC<{ justification: string }> = ({ justification }) => {
    ...
    if (!hasHistory) {
        return (
            <div className="text-sm text-purple-700 dark:text-purple-300">
                <p>Ainda não há dados de estudo para gerar uma sugestão detalhada. A recomendação é baseada na relevância que você definiu para os tópicos.</p>
            </div>
        );
    }
    ...
    details = details.map(detail => {
        ...
        if (name === 'Frequência') {
            if (value === '0x') value = 'Nenhuma vez';
            else value = value.replace('x', ' vezes');
        }
        if (name === 'Último Estudo') {
            if (value.includes('999d')) value = 'Nunca estudado';
            else value = value.replace('d', ' dias atrás');
        }
        if (name === 'Taxa de Acertos') {
            if (!hasStudied) value = 'Sem histórico';
        }
        return { name, value };
    });
...
const StudyRegisterModal: React.FC<StudyRegisterModalProps> = ({
  ...
  const formatTime = (ms: number): string => {
    if (isNaN(ms) || ms < 0) {
      ms = 0; // Garante que o tempo não seja negativo ou inválido
    }
  ...
  const [countInPlanning, setCountInPlanning] = useState(false);
  ...
  const [isAddReviewModalOpen, setIsAddReviewModalOpen] = useState(false); // Estado para o novo modal
  ...
  // Função para lidar com a seleção de data pelos botões
  const handleDateSelect = (type: 'today' | 'yesterday') => {
    const today = new Date();
    let dateToSet = today;
    if (type === 'yesterday') {
      dateToSet.setDate(today.getDate() - 1);
    }
    setSelectedDate(dateToSet.toISOString().split('T')[0]);
    setShowDatePicker(false); // Esconde o date picker manual ao selecionar hoje/ontem
  };
  ...
  // 1. Busca os dados do edital (disciplinas e tópicos)
  useEffect(() => {
  ...
  // 2. Atualiza a lista de tópicos quando uma disciplina é selecionada ou a lista de disciplinas muda
  useEffect(() => {
    if (selectedSubject) {
  ...
  // 3. Preenche o formulário quando o modal abre ou os dados iniciais são alterados
  useEffect(() => {
    if (isOpen) {
      resetForm(); // Garante que o formulário esteja limpo antes de preencher

      // Caso 1: Editando um registro existente ou preenchendo com recomendação
      if (initialRecord) {
        ...
        setCountInPlanning(initialRecord.countInPlanning ?? true); // Default to true for recommendations
        ...
        setSelectedDate(initialRecord.date || new Date().toISOString().split('T')[0]);
        
        const today = new Date().toISOString().split('T')[0];
        const yesterday = new Date(new Date().setDate(new Date().getDate() - 1)).toISOString().split('T')[0];
        setShowDatePicker(initialRecord.date && initialRecord.date !== today && initialRecord.date !== yesterday);
        
        setQuestions(initialRecord.questions ? [{ correct: initialRecord.questions.correct, incorrect: (initialRecord.questions.total - initialRecord.questions.correct) }] : [{ correct: 0, incorrect: 0 }]);
      ...
      // Caso 2: Novo registro a partir da página Edital
      ...
      // Caso 3: Novo registro em branco (ou vindo do cronômetro)
      else {
        setStudyTime(formatTime(initialStudyTime || 0));
      }
    }
  ...
  // 4. Controla o fechamento dos dropdowns ao clicar fora
  useEffect(() => {
  ...
  const handleSave = () => {
    ...
    const studyRecord: StudyRecord = {
      id: initialRecord?.id, // O ID será gerado no DataContext se for um novo registro
      date: selectedDate,
      ...
      questions: {
        ...
        total: questions[0].correct + questions[0].incorrect,
      },
      notes: comments,
  ...
  // Manipulador para salvar a revisão do modal
